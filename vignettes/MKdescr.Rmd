...
the (standardized) IQR, respectively [see Arachchige2020].
```{r}
## 5% outliers
...
## z-Score
There are functions to compute z-score and robust variants of the z-score based
...
## z-Factor
There are functions to compute z-factor and robust variants of the z-factor based
on median and MAD, respectively median and IQR. The functions can also be used
to compute z'-factors [see @Zhang1999].
```{r}
x <- rnorm(100, mean = 10, sd = 2)
y <- rnorm(100, mean = 40, sd = 2)
zfactor(x, y)
medZfactor(x, y)
iqrZfactor(x, y)
```
