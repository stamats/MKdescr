illustrate.boxplot <- function(x){
  ...
  gg <- ggplot(DF, aes(x = .data$x, y = .data$y)) +
    ...
    geom_vline(xintercept = upp.fence, linetype = "dotted",
               color = "#5C88DA") + 
    geom_point(aes(x = .data$x, y = .data$y), data = DFiqr, pch = "|", size = 10, 
               position = position_nudge(y=0.03), color = "#CC0C00") +
    geom_line(data = DFiqr, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
    annotate(geom = "text", x = mean(iqr), y = 1.6, label = "middle 50%",
             color = "#CC0C00") +
    geom_point(aes(x = .data$x, y = .data$y), data = DFmin, pch = "|", size = 10, 
               position = position_nudge(y=0.03), color = "#CC0C00") +
    geom_line(data = DFmin, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
    annotate(geom = "text", x = mean(DFmin$x), y = 1.6, label = "lower 25%",
             color = "#CC0C00") +
    geom_point(aes(x = .data$x, y = .data$y), data = DFmax, pch = "|", size = 10, 
               position = position_nudge(y=0.03), color = "#CC0C00") +
    geom_line(data = DFmax, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
    annotate(geom = "text", x = mean(DFmax$x), y = 1.6, label = "upper 25%",
             color = "#CC0C00") + 
    geom_point(aes(x = .data$x, y = .data$y), data = DFwhisker, pch = "|", size = 10, 
               position = position_nudge(y=0.03), color = "#CC0C00") +
    geom_line(data = DFwhisker, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
    annotate(geom = "text", x = mean(iqr), y = 1.85, 
             label = "maximum range of whiskers", color = "#CC0C00") +
    geom_line(data = DFout1, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
    annotate(geom = "text", x = mean(DFout1$x), y = 1.85, 
             label = "outlier region", color = "#CC0C00") +
    geom_line(data = DFout2, aes(x = .data$x, y = .data$y), 
              color = "#CC0C00", position = position_nudge(y = 0.02)) +
