illustrate.quantile <- function(x, alpha, type){
  ...
  if(length(type) > 1){
    ...
    gg <- gg + geom_point(data = DFqR, aes_string(x = "x", y = "y"), pch = 8, size = 3, 
                          color = "#5C88DA") +
  ...
  if(length(quant) == 2){
    gg <- gg + geom_line(data = DFq, aes_string(x = "quant", y = "y"), 
